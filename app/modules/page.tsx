...
interface Module {
  code: string; name: string; grade: number;
  letter: string; credits: number; progress: number; color: string;
}

interface Assignment {
  id: number; name: string; module_code: string; due: string; urgency: string;
}
...
export default function ModulesPage() {
  ...
  const filteredAssignments = selected
    ? assignments.filter(a => a.module_code === selected)
    : assignments;
  ...
  return (
    <main style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ color: 'var(--muted)', fontSize: '15px' }}>Click a module to filter assignments</p>
      </div>
        ...
        {modules.map((m, i) => {
          ...
          return (
            <div
              key={m.code}
              className="fade-up"
              onClick={() => setSelected(isSelected ? null : m.code)}
              style={{
            ...
            >
              ...
              <div style={{ background: 'var(--surface2)', borderRadius: '4px', height: '5px', overflow: 'hidden', marginBottom: '8px' }}>
                <div style={{
                  width: `${m.progress}%`, height: '100%', borderRadius: '4px',
                  background: m.color, transition: 'width 0.6s ease',
                }} />
              </div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--hint)', display: 'flex', justifyContent: 'space-between' }}>
